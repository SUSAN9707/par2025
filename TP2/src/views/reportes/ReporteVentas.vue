...
import { getReporteVentas } from '@/service/reportes.service'
...
const reporte = ref([])
const now = new Date()
const fechaInicio = ref(new Date(now.getFullYear(), now.getMonth(), 1, 0, 0, 0))
const fechaFin = ref(new Date(now.getFullYear(), now.getMonth() + 1, 0, 23, 59, 59))
...
async function cargarReporte() {
    try {
        ...
        const response = await getReporteVentas(fechaInicio.value, fechaFin.value)
...
<template>
    <div>
        <div class="card">
            <Toolbar class="mb-4">
                <template #start>
                    <span class="text-xl font-bold">Reporte de Ventas</span>
                </template>
                <template #end>

...
import { getTopClientes } from '@/service/reportes.service'
...
async function cargarDatos() {
    try {
        const res = await getTopClientes(fechaInicio.value, fechaFin.value)
        rawData.value = res.data.data

        chartData.value = {
            labels: rawData.value.map(c => c.nombre),
            datasets: [
                {
                    label: 'Total Vendido',
                    backgroundColor: getComputedStyle(document.documentElement).getPropertyValue('--primary-color') || '#10b981',
                    data: rawData.value.map(c => c.total)
                }
            ]
        }
...
function exportarPDF() {
    ...
    if (chartCanvas) {
        ...
        doc.addPage()
        doc.setFontSize(14)
        doc.text('Gráfico de Top Clientes', 14, 20)
        doc.addImage(chartImage, 'PNG', 15, 30, 180, 100)
    }

    doc.save('top_clientes.pdf')
}
</script>
...
<template>
    <div class="card">
        <Toolbar class="mb-4">
            <template #start>
                <span class="text-xl font-bold">Top 15 Clientes</span>
            </template>
            <template #end>
        ...
        <div v-if="rawData.length > 0">
            ...
            <DataTable :value="rawData" responsiveLayout="scroll">
                ...
                <Column field="total" header="Total Vendido" />
            </DataTable>
        </div>

        <div v-else class="text-center italic text-gray-500 py-6">
            No se encontraron clientes con ventas en este rango de fechas.
        </div>
    </div>
</template>
